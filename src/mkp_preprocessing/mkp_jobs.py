#mkp_router/src/mkp_preprocessing/mkp_jobs.py
...
logger = logging.getLogger("mkp_jobs")
...
def processar_mkp(tenant_id, file_path, descricao):
    job = get_current_job()
    ...
    try:
        comando = [
            "python3",
            "-u",
            "src/mkp_preprocessing/main_mkp_preprocessing.py",
        ...
        if proc.returncode != 0:
            raise RuntimeError(
                f"main_mkp_preprocessing retornou código {proc.returncode}"
            )
        ...
        # ----------------------------------------------------
        writer.salvar_historico_mkp_job(
            tenant_id=tenant_id,
            job_id=job_id,
