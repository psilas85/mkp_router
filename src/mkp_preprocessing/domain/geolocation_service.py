#mkp_router/src/mkp_preprocessing/domain/geolocation_service.py
...
from mkp_preprocessing.domain.address_normalizer import (
    normalize_base,
    normalize_for_cache,
    normalize_for_geocoding,
)
from mkp_preprocessing.domain.capital_polygon_validator import (
...
class GeolocationService:
    ...
    # ============================================================
    # 🌍 Busca principal (BASE ANTIGA: valida só CAPITAL)
    # ============================================================
    def buscar_coordenadas(self, endereco: Optional[str], cep: Optional[str] = None):
        ...
        is_capital = self.is_capital(cidade_input, uf_input)
        ...
        try:
            ...
            if cache_db:
                ...
                # 🔒 Capital definida pelo INPUT -> valida polígono
                if is_capital:
                    ...
                    # ✅ Interior: aceita sem validar município
                    with self.stats_lock:
                        self.stats["cache_db"] += 1
                    return lat, lon, "cache_db"
        ...
        # ==========================================================
        lat, lon, _, _ = self._buscar_nominatim_estruturado(endereco_normalizado)
            ...
            if is_capital:
                if not ponto_dentro_capital(lat, lon, cidade_input, uf_input):
                    ...
                    self._salvar_cache_detalhado(
                        cache_key, raw, endereco_normalizado,
                        lat, lon, "nominatim_struct", trace
                    )
                    with self.stats_lock:
        ...
        # 3) GOOGLE (fallback)
        # ==========================================================
        ENABLE_GOOGLE = os.getenv("ENABLE_GOOGLE_GEOCODING", "true").lower() == "true"
        if not ENABLE_GOOGLE:
            logger.info(f"[{trace}][GOOGLE][DISABLED_BY_FLAG]")
            return None, None, "google_disabled"
    ...
    def geocodificar_em_lote(
        ...
        from mkp_preprocessing.domain.utils_texto import fix_encoding
