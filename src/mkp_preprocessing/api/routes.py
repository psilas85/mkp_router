#sales_router/src/mkp_preprocessing/api/routes.py
...
from mkp_preprocessing.visualization.mkp_plotting import (
    buscar_pdvs,
    gerar_mapa_pdvs,
)
...
def reprocessar_input(request: Request, input_id: str = Query(...), descricao: str = Query(...)):
    ...
    try:
        ...
        conn_redis = Redis(host="redis", port=6379)
        queue = Queue("pdv_jobs", connection=conn_redis)
        job = queue.enqueue(processar_mkp, tenant_id, arquivo, descricao)
